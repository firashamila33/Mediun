const extractArticleMedia = description => {
  ...
  if (indexOfVideoIndicator > -1) {
    videosub = description.substring(
      description.indexOf(videoIndicator) + videoIndicator.length
    );
    videoLink = videosub.substring(0, videosub.indexOf(`"`));
    if (extractYoutubeVideoID(videoLink)) {
      containsVideo = true;
      emeddedYoutubeLink = `https://www.youtube.com/embed/${extractYoutubeVideoID(
        videoLink
      )}`; // converting to embedded Url
    }
  }
  ...
  if (indexOfimageIndicator > -1) {
    imagesub = description.substring(
      description.indexOf(imageIndicator) + imageIndicator.length
    );
