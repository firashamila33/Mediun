  ...
  Query: {
    ...
    articleFeed: async (channel, { cursor }) => {
      ...
      if (!cursor || parseInt(cursor) === -1) {
        const newestArticle = await Article.find()
          .sort({ createdAt: -1 })
          .limit(1);
      ...
      const articlesResult = await Article.find({ createdAt: { $lte: cursor } })
        .sort({ createdAt: -1 })
        .limit(feedSize + 1);
  ...
  Subscription: {
    articleAdded: {
      subscribe: withFilter(
        () => pubsub.asyncIterator("articleAdded"),
        (payload, variables) => {
          return true;
        }
      )
    ...
    articleDeleted: {
      subscribe: withFilter(
        () => pubsub.asyncIterator("articleDeleted"),
        (payload, variables) => {
          return true;
        }
      )
    ...
    articleEdited: {
      subscribe: withFilter(
        () => pubsub.asyncIterator("articleEdited"),
        (payload, variables) => {
          return true;
        }
      )
